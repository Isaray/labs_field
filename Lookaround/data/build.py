...
def build_transforms(cfg, method):
    ...
    if cfg.DATASET.NAME == 'cifar100':
        ...
        if method == 'RandomErasing':
            return transforms.Compose([
                transforms.RandomCrop(cfg.DATASET.IMAGESIZE, padding=4),
                transforms.RandomErasing(p=0.5, scale=(0.02, 0.33), ratio=(0.1, 3.3), value=0, inplace=False),
                transforms.ToTensor(),
                transforms.Normalize(mean, std),
            ])
        if method == 'test' or method == 'validation':
            return transforms.Compose([
                transforms.ToTensor(),
                transforms.Normalize(mean, std),
            ])
...
def build_dataloader(cfg, transform, data_dir, is_train,is_valid=False, batchsize=100):
    ...
    train_dataset, valid_dataset = split_dataset_by_class(dataset, train_ratio=0.8)
    
    if is_train == True :
        dataset=train_dataset
        # return train_dataset
    if is_train == False and is_valid==True:
        # 划分训练集和验证集
    ...
    if is_train == True:
        if cfg.TRAIN_MODE == "TRAIN_LOOKAROUND":
            ...
            dataloader = torch.utils.data.DataLoader(
                dataset, batch_size=cfg.SOLVER.BATCH_SIZE, sampler=sampler, num_workers=32, drop_last=True, pin_memory=True)
        else:
            sampler = None
