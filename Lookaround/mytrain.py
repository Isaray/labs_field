...
def train_imagenet_lookaround(cfg, is_image_net=True):
    ...
    #                   'AutoAugment', 'RandAugment']
    TRANSFORM_list = ['RandomHorizontalFlip',
                      'AutoAugment','RandomVerticalFlip', 'RandAugment',"RandomRotation"]
    # trainloaders = [build_dataloader(cfg, build_transforms(
    #     cfg, transform), cfg.DATA_DIR, is_train=True) for transform in TRANSFORM_list]
    ...
    test_t = build_transforms(cfg, 'test')
    testloader = build_dataloader(cfg, test_t, cfg.DATA_DIR, is_train=False)
    validloader = build_dataloader(cfg, test_t, cfg.DATA_DIR, is_train=False,is_valid=True)
    train_iter1 = iter(trainloaders[0])
    model = init_model(cfg)
    device_ids = cfg.MODEL.GPU_IDS
    if is_image_net or cfg.DATASET.IMAGESIZE == 224:
    ...
    output_dir = cfg.OUTPUT_DIR + \
        args.yaml_path[0:-5] + '/'+f'TRAIN_Lookaround'+ '/'+args.tag
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    logger = setup_logger(f"output_dir {cfg.TRAIN_MODE}", output_dir, 0)
    logger.info("Running with config:\n{}".format(cfg))
    ...
    for epoch in range(cfg.SOLVER.MAX_EPOCHS):
        ...
        train_iter3 = iter(trainloaders[2])
        #add
        train_iter4 = iter(trainloaders[3])
        train_iter5 = iter(trainloaders[4])
        logger.info(f"train model:  epoch: {epoch}")
        model.train()
        train_loss = 0
        correct1 = 0
...
def train_imagenet_lookahead(cfg, is_image_net=True):
    ...
    output_dir = cfg.OUTPUT_DIR + args.yaml_path[0:-5] + '/'+f'TRAIN_LookAhead'+ '/'+args.tag
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    logger = setup_logger(f"output_dir {cfg.TRAIN_MODE}", output_dir, 0)
    logger.info("Running with config:\n{}".format(cfg))
...
def train_imagenet_lookaroundselect(cfg, is_image_net=True):
    device = 'cuda:' + str(cfg.MODEL.DEVICE_ID)
    # TRANSFORM_list = ['RandomHorizontalFlip',
    #                   'AutoAugment', 'RandAugment']
    TRANSFORM_list = ['RandomHorizontalFlip',
    ...
    for method in TRANSFORM_list:
        TRANSFORM_dict[method]=[]
        TRANSFORM_score.append(0)
    trainloaders = [build_dataloader(cfg, build_transforms(
        cfg, transform), cfg.DATA_DIR, is_train=True) for transform in TRANSFORM_list]
    ...
    output_dir = cfg.OUTPUT_DIR + \
        args.yaml_path[0:-5] + '/'+f'TRAIN_LookaroundSelect'+ '/'+args.tag
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    logger = setup_logger(f"output_dir {cfg.TRAIN_MODE}", output_dir, 0)
    logger.info("Running with config:\n{}".format(cfg))
    ...
    for epoch in range(cfg.SOLVER.MAX_EPOCHS):
        ...
        for batch_idx in tbar:
            ...
            if 'select' in args.tag:
                if batch_idx<10:
                    ...
                    TRANSFORM_score_all.append(TRANSFORM_score) 
                else:
                    num_samples = 3  # 指定要选取的元素数量
                    #三个最高的Score
                    TRANSFORM_score=[]
                    for method in TRANSFORM_list:
                        ...
                        TRANSFORM_score.append(ucb)
                    TRANSFORM_score_all.append(TRANSFORM_score) 
                    iters=[train_iter1, train_iter2, train_iter3,train_iter4, train_iter5]
                    TRANSFORM_list_new=[TRANSFORM_list[m] for m in np.argsort(TRANSFORM_score)[-3:]]  
                    train_iter_list=[iters[m] for m in np.argsort(TRANSFORM_score)[-3:]] 
                # train_iter_list = random.sample(train_iter_list, num_samples)
                    # print(TRANSFORM_list_new,np.argsort(TRANSFORM_score)[:-3])
                TRANSFORM_list_all.append(TRANSFORM_list_new)
            # val_acc_temp=[]#想计算分数的方法
            for idx,x in enumerate(train_iter_list):
                s = time.time()
                inputs, targets = next(x)
                iter_time += time.time() - s
                s = time.time()
                ...
                loss.backward()
                optimizer.step()
                #需要在这里测试validation数据集，不更新参数只是看一下结果
                val_loss = 0
                val_correct1 = 0
                val_correct5 = 0
                val_total = 0
                with torch.no_grad():
                    # print(len(validloader.dataset),len(validloader))
                    # 使用 random.choice() 函数随机选择一个 batch
                    val_inputs, val_targets = random.choice(validdataloader_list)
                    # val_inputs, val_targets = random.choice(iter(validloader))
                    # print(len(val_inputs))
                    val_inputs = val_inputs.to(device)
                    val_targets = val_targets.to(device)
                    val_outputs = model(val_inputs)
                    val_acc1,_= accuracy(val_outputs, val_targets, topk=(1,5))
                    TRANSFORM_dict[TRANSFORM_list_new[idx]].append(val_acc1.cpu())
        ...
        with torch.no_grad():
            ...
            for batch_idx, (inputs, targets) in enumerate(tbar):
                ...
                val_acc1, val_acc5 = accuracy(outputs, targets, topk=(1, 5))
                val_correct1 += val_acc1.item() * len(inputs) / 100
                val_correct5 += val_acc5.item() * len(inputs) / 100
                tbar.set_postfix(
                    {"Acc1": round(100.*val_correct1/val_total, 3), "Acc5": round(100.*val_correct5/val_total, 3)})
        myacc = (100.*val_correct1/val_total, 100.*val_correct5/val_total)
        val_acc.append(myacc)
        logger.info(
            f"lr: {optimizer.state_dict()['param_groups'][0]['lr']},val loss:{round(val_loss/(batch_idx+1),3)},val_acc1:{round(100.*correct1/total,3)},val_acc5:{round(100.*correct5/total,3)}")
        ...
        torch.save(checkpoint, output_dir + f'/last_{cfg.MODEL.NAME}.ckpt')
        np.save(output_dir + '/test_acc.npy', np.array(test_acc))
        np.save(output_dir + '/val_acc.npy', np.array(val_acc))
        np.save(output_dir + '/TRANSFORM_score_all.npy', np.array(TRANSFORM_score_all))
        print(1)
        with open(output_dir + '/transform_method.txt', 'w') as f:  
            json.dump(TRANSFORM_list_all, f)
    return
...
if __name__ == '__main__':
    ...
    if args.train_mode == 'TRAIN_LOOKAROUND':
        ...
        train_imagenet_lookaroundselect(cfg, is_image_net=is_imagenet)